...
; ----- Toast configuration -----
;toastSecs := 2          ; seconds the toast stays visible (change if you want)

; ----- Helpers: toast (TrayTip) -----
;ShowModeToast(text) {
;    global toastSecs
;    TrayTip, Helldivers2, %text%, % toastSecs
;}

; Clear tray tip (optional helper)
;ClearModeToast() {
;    TrayTip
;}
...
CheckF11F12() {
    ...
    if (both && !F11F12_active) {
        ...
        ;ShowModeToast("WASD → Arrows (F11+F12)")
    }
...
if (!ToggleEnabled) {
    DisableAll()
    ;ShowModeToast("Script: DISABLED")
} else {
    EnableFromCurrentState()
    ;if (F11F12_active || GetKeyState("LControl", "P"))
    ;    ShowModeToast("WASD → Arrows")
    ;else
    ;    ShowModeToast("WASD → IJKL")
}
return
